...
const formatTimestamp = (isoString) => {
  if (!isoString) return 'N/A'; // Handle missing data
  try {
...
export default function DataPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Add a formatted timestamp (if created_at exists) or use ID
      const formattedData = sensorData.map(item => ({
        ...item,
        timestamp: formatTimestamp(item.created_at) // 'created_at' is added by default by Supabase
      })).reverse(); // Reverse so time flows left-to-right in charts

      setData(formattedData);
      setError(null);
    } catch (err) {
      console.error('Error fetching data:', err);
      // Check if the error is the one we saw before
      if (err.message && err.message.includes("column sensor_data.created_at does not exist")) {
         setError("Data is loading, but timestamp column is missing. Please re-create Supabase table and check the 'created_at' box.");
         // Fallback: try fetching without ordering by created_at
         const { data: fallbackData } = await supabase.from('sensor_data').select('*').limit(50);
         if(fallbackData) setData(fallbackData.reverse());
      } else {
  ...
  if (loading) {
   ...
   const chartData = data.map(d => ({
    timestamp: d.timestamp || d.id, // Use formatted time or fall back to ID
        ...
        <div className={styles.chartBox}>
          ...
          <ResponsiveContainer width="100%" height="90%">
            <LineChart data={chartData}>
              ...
              <XAxis dataKey="timestamp" stroke="var(--text-secondary)" fontSize="12px" />
          ...
          <table className={styles.table}>
            <thead>
              <tr className={styles.tr}>
                <th className={styles.th}>Timestamp (or ID)</th>
            ...
            <tbody>
              {data.slice().reverse().slice(0, 10).map((row) => ( // Show latest 10, newest first
                <tr key={row.id} className={styles.tr}>
                  <td className={styles.td}>{row.timestamp || row.id}</td>
